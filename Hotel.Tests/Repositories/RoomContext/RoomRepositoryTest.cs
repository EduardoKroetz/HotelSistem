...
{
  private static RoomRepository RoomRepository { get; set; } = null!;

  [ClassInitialize]
  public static async Task Startup(TestContext? context)
  {
    await Startup();
    RoomRepository = new RoomRepository(MockConnection.Context);
  }
  ...
  {
    var rooms = await RoomRepository.GetByIdAsync(Rooms[0].Id);

    Assert.IsNotNull(rooms);
    Assert.AreEqual(Rooms[0].Id, rooms.Id);
    Assert.AreEqual(Rooms[0].Number, rooms.Number);
    Assert.AreEqual(Rooms[0].Description, rooms.Description);
    Assert.AreEqual(Rooms[0].Price, rooms.Price);
    Assert.AreEqual(Rooms[0].Status, rooms.Status);
    Assert.AreEqual(Rooms[0].Capacity, rooms.Capacity);
    Assert.AreEqual(Rooms[0].Description, rooms.Description);
  }
  ...
  {
    var parameters = new RoomQueryParameters(0, 100, Rooms[0].Number, null, Rooms[0].Price, null, Rooms[0].Status, null, null, null, null, null, null);
    var rooms = await RoomRepository.GetAsync(parameters);

    var room = Rooms.ToList()[0];

    Assert.IsNotNull(room);
    Assert.AreEqual(Rooms[0].Id, room.Id);
    Assert.AreEqual(Rooms[0].Number, room.Number);
    Assert.AreEqual(Rooms[0].Description, room.Description);
    Assert.AreEqual(Rooms[0].Price, room.Price);
    Assert.AreEqual(Rooms[0].Status, room.Status);
    Assert.AreEqual(Rooms[0].Capacity, room.Capacity);
    Assert.AreEqual(Rooms[0].Description, room.Description);
    Assert.AreEqual(Rooms[0].Services, room.Services);
    Assert.AreEqual(Rooms[0].Images, room.Images);
  }
  ...
  {
    var parameters = new RoomQueryParameters(0, 100, null, null, null, null, null, null, null, Services[0].Id, null, null, null);
    var rooms = await RoomRepository.GetAsync(parameters);
    ...
    foreach (var room in rooms)
    {
      var hasService = await MockConnection.Context.Rooms
  ...
  {
    var parameters = new RoomQueryParameters(0, 100, null, null, null, null, null, null, null, null, Categories[0].Id, null, null);
    var rooms = await RoomRepository.GetAsync(parameters);

    Assert.IsTrue(rooms.Any());

    foreach (var room in rooms)
      Assert.AreEqual(Categories[0].Id, room.CategoryId);
  ...
  {
    var parameters = new RoomQueryParameters(0, 100, null, null, null, null, null, null, null, null, null, Rooms[0].CreatedAt, "eq");
    var rooms = await RoomRepository.GetAsync(parameters);

    Assert.IsTrue(rooms.Any());
    foreach (var room in rooms)
      Assert.AreEqual(Employees[0].CreatedAt.Date, room.CreatedAt.Date);
  }


  [TestMethod]
  public async Task GetAsync_WhereNumberGratherThan5_And_PriceLessThan60_And_StatusEqualsAvailable_And_CapacityGratherThan2_ReturnRooms()
  {
    var parameters = new RoomQueryParameters(0, 100, 5, "gt", 60, "lt", ERoomStatus.OutOfService, 2, "gt", null, null, null,null);
    var rooms = await RoomRepository.GetAsync(parameters);
